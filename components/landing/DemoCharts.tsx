    ...
    <div className="space-y-3">
      ...
      >
        ...
        {coordinates.map((point) => (
          <g key={point.label}>
            <circle cx={point.x} cy={point.y} r="4.5" fill="#FCF7EE" stroke="#D35F39" strokeWidth="2" />
            <text
              x={point.x}
              y={height - 6}
              textAnchor="middle"
      ...
      <div
        className="grid gap-2 text-[11px] font-medium text-text-muted"
        style={{ gridTemplateColumns: `repeat(${points.length}, minmax(0, 1fr))` }}
      >
        {points.map((point) => (
          <div key={point.label} className="rounded-[0.9rem] bg-surface-soft/70 px-2 py-2 text-center">
            <p>{point.label}</p>
...
export function MiniBarChart({
  ...
  return (
    <div
      role="img"
      aria-label={ariaLabel}
      className="flex items-end gap-3 rounded-[var(--radius-card)] border border-border/65 bg-[#FFF9EF] px-4 py-4"
    >
      {points.map((point) => (
